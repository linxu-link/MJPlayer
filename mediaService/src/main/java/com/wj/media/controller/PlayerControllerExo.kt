package com.wj.media.controller
...
/**
 * ExoplayerControllerImpl 是 ExoPlayer 的具体实现类，继承自 IVideoPlayerController
 */
@UnstableApi
class ExoplayerControllerImpl(
    ...
    private fun handlePlaybackStateChange(isPlaying: Boolean, state: PlaybackState) {
        updateState { copy(isPlaying = isPlaying, playbackState = state) }
    }

    private fun handlePositionUpdate(position: Long, duration: Long) {
        updateState { copy(currentPosition = position, duration = duration) }
    }

    private fun handleBufferingUpdate(percentage: Int) {
        updateState { copy(bufferedPercentage = percentage) }
    }

    private fun handlePlaybackSpeedUpdate(speed: Float) {
        updateState { copy(playbackSpeed = speed) }
    }
